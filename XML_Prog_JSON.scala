...
import org.json.XML  //need to import json jar https://mvnrepository.com/artifact/org.json/json/20160810
...
println(XML.toJSONObject(xml1).toString)

val list = parseMyXML(xml1, requirement)
//list.map(x => List(x.comment.title,x.comment.description,x.author.author_name,x.author.author_date,x.publisher.publisher_name,x.publisher.publisher_date )).foreach(println)
...
val xml_parsed_rdd = rdd.map(x=>(x.ID_col1, x.Col2, x.Col3, x.XML_Col4, parseMyXML(x.XML_Col4, requirement)))
val flattened_rdd = xml_parsed_rdd.flatMap{case(col1,col2,col3,col4,xmlRows) => xmlRows.map(titleRow=>(col1,col2,col3,col4)->titleRow)}
val flattened_list = flattened_rdd.map{case(col1,titleRow) =>(col1._1,col1._2,col1._3,titleRow.comment.title,titleRow.comment.description,titleRow.author.author_name,titleRow.author.author_date,titleRow.publisher.publisher_name,titleRow.publisher.publisher_date,XML.toJSONObject(col1._4).toString)}
